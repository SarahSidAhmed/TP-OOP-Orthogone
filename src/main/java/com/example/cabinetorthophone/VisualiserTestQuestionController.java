...
public class VisualiserTestQuestionController implements Initializable {
    private Stage stage;
    private Scene scene;
    ...
    @FXML TableView<Test_Question> tableTestQuestion;
    @FXML TableColumn<Test_Question, Void> tableColumnCheck;
    ...
    protected void deleteData(ActionEvent event){
        TableView.TableViewSelectionModel<Test_Question> selectionModel = tableTestQuestion.getSelectionModel();
        ...
        for (int i= selectedIndeces.length -1; i>=0; i-- ){
            ...
            tableTestQuestion.getItems().remove(selectedIndeces[i].intValue());
    ...
    // not finished yet
    public void ajouterTestQuestion(ActionEvent actionEvent)throws IOException {
    ...
    private void addButtonToTable() {
        Callback<TableColumn<Test_Question, Void>, TableCell<Test_Question, Void>> cellFactory = new Callback<TableColumn<Test_Question, Void>, TableCell<Test_Question, Void>>() {
            @Override
            public TableCell<Test_Question, Void> call(final TableColumn<Test_Question, Void> param) {
                final TableCell<Test_Question, Void> cell = new TableCell<Test_Question, Void>() {
...

                    {
                        ...
                        btn.setOnAction((event) -> {
                            Test_Question data = getTableView().getItems().get(getIndex());
